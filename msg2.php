 	...
 	if($res){
 		...
 		while($row = mysqli_fetch_assoc($res))
			...
			while($rows = $res2->fetch_assoc()) {
  			...
  			$final = 'SELECT DISTINCT(message.msg),message.senderID, message.recvID , message.msgType, mID FROM `message` JOIN `user` WHERE (message.`senderID` = user.user_id OR message.`recvID` = user.user_id)  and (message.senderID = "'.$response.'" and message.recvID = "'.$response2.'" OR message.senderID = "'.$response2.'" and message.recvID = "'.$response.'") ORDER by message.`mID` ASC';
